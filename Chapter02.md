...
事实上,这个嵌入式操作系统很不同,因为我们向内存里加载了一整个二进制的映像.多数的系统都有一个不可变的内存段(Flash/ROM),用于存放可执行代码.系统中还会有一个可变的内存段(RAM),用于存放变量数据.变量存在于RAM中,每一个都有一个相应的内存中的位置.当编译一个存在Flash中并使用RAM储存变量的可执行程序时,我们需要复制一份初始值给Flash中的变量,以便系统每一次启动时,可以用初始值来把这些变量给初始化.我们还需要存在于Flash中的代码来把这些值复制到变量里去.当然了,在main()函数被调用*之前*.
...
接下来,我们来看看反汇编以后的代码.不要问我反汇编器在哪里.(命令就是arm-none-eabi-objdump).onjdump会帮你完成所有的事情,且无需指定目标处理器.在示例代码里有disassemble.bat和disassemble.sh,你可以使用它们来反编译代码.执行完毕后,你应该会得到一个kernel.asm文件,内容如下(可能略有不同,RPi2):
...
可执行映像末尾处的值可以用十六进制编辑器来查看。![Hex Editor View](/img/armc-04-kernel-img-hex-editor.jpg)下一行却把PC+120里的'地址'放进了r3里，此处的值为0x10090，居然超出了映像文件大小。这个变量的之将会是未定义的。这就是tim variable（我也不知道是什么），它事实上不会被初始化为任何值，直到我们在第73行的一个for循环开始使用它。（如果你了解一些C语言标准的话，肯定就会知道这是个不正确的行为，这事儿过一会再讲）
...
我们确实看到了linker脚本中的一个“bug”：这种偏移是不必要的。偏移量是正确的是因为当我们在RAM中加载映像时数据确实是会加上0x8000，这是引导加载器将要放置映像的地方，但有些东西将data段与常量数据分隔开了。加上一个verbose选项就可以知道linker用的是什么脚本：
...
虽然代码是一样的，但是你需要在编译指令后面加上`-Wl,-T,rpi.x`选项来告诉LD程序：我们要使用一个不同的链接脚本，而不是内置脚本。我们依然保留`-Wl,verbose`选项来观察发生了什么。当你在向GCC传递编译参数时，最好去参考一下[GCC文档](http://gcc.gnu.org/onlinedocs/gcc/Link-Options.html)，它会告诉你这些选项会指定哪些细节。现在链接器脚本已经在我们的自己的掌控下了。我们可以试着找出哪里出错了。我说错了，虽然从技术上讲这是可行的，但有一个讨厌的0x8000偏移量。调试的痛苦我深有感触。让我们看看我们需要做些什么来修复它。你还可以看到，当linker脚本需要处理c++时，它会变得多么复杂!

我希望你依然跟得上我的叙述!我知道这似乎和RPI开发扯得有点远了,但事实上，知道工具是怎么构建代码的对于以后的事是至关重要的——如果不了解一些这方面的东西，你以后是没办法自己做这种事的。

当我浏览linker脚本的时候，我通常会去寻找一些比较突出的东西。那里面大部分的程序段都没有被我纳入我们的实例，

综上，问题一定处在__data_段的开头和text段的结尾（main符号标志着text段的开始）。找了一会以后，我在第101行发现了一串注释和一行有关DATA段的align操作。
